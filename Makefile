-include .env
.DEFAULT_GOAL:=help
.PHONY: start stop migrate destroy
DOCKER_COMPOSE_AVAILABLE := $(shell docker compose 2>/dev/null)

init: req copy-env ## Init the project
	docker compose build --no-cache --progress plain
	# init node modules since they are not synchronized because of their large size
	cd ui; npm i;
	cd api;npm i;
...
seeds: ## Will reload seeds into the local db
	@docker compose exec api npm run prisma:migrations:apply
